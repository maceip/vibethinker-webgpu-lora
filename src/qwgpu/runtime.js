...
export class QwenWGPU {
  ...
  rmsT(enc, xBuf, gBuf, yBuf, T, K) {
    ...
    const uni = this._staticUni(`rmsT:${K}:${this.cfg.rmsNormEps}`, new Uint8Array(u));
    this._dispatch(enc, this.pipes.rmsT, this._bgCached(this.pipes.rmsT, [xBuf, gBuf, yBuf, uni], `rmsT:${K}`), T, 1, 'rmsT');
  }
  ...
  _ensurePrefillScratch(T, loraRank = 0) {
    ...
    this.sTcap = T; this.sTLoraRank = loraRank;
  }
