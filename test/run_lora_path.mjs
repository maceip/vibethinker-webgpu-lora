...
const adapterDir = process.env.ADAPTER_DIR || 'adapters/highconf-trace-20260623';
...
await page.goto(`http://localhost:8013/test/lora_load_path.html?dir=${encodeURIComponent(adapterDir)}`, { waitUntil: 'domcontentloaded' });
const t0 = Date.now();
...
await browser.close();
if (!lines.some(l => l.includes('LORA-LOAD-PATH ALL PASS'))) process.exitCode = 1;
