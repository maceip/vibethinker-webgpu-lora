...
async function requestDevice() {
  ...
  if (!adapter.features.has('subgroups')) throw new Error('GPU lacks required "subgroups" feature; no fallback kernels are bundled');
...
async function tryFetchAdapter(dir) {
  ...
  const wrap = async (resp, name) => {
    ...
    return { name, async text() { return new TextDecoder().decode(buf); }, async arrayBuffer() { return buf; } };
  };
...
window.run = async () => {
  ...
  row({ type: 'device', timestampQuery: hasTimestamp, maxBufferSize: adapter.limits.maxBufferSize, maxStorageBufferBindingSize: adapter.limits.maxStorageBufferBindingSize });
  const ref = await (await fetch('./ref.json')).json();

  const rt = new QwenWGPU(dev, QWEN25_3B, { decodeBatchSize: 16, maxDecodeBatchSize: 32, samplingTopK: 40, maxSamplingTopK: 64 });
  const tBuild = performance.now();
  await rt.build('/model');
  row({ type: 'load', seconds: (performance.now() - tBuild) / 1000, kvBytes: rt.estimateKvCacheBytes(), pool: rt.poolStats() });

  const tuning = await rt.autotuneDecodeBatch();
  row({ type: 'decode-autotune', ...tuning });

  for (const L of [64, 256, 1024, 4096, 8192]) {
    if (L > rt.maxPrefillT) { row({ type: 'prefill', L, skipped: 'exceeds maxPrefillT' }); continue; }
    const t0 = performance.now();
    rt.prefillBatch(tile(ref.ids, L));
    await rt.argmaxLogits();
    row({ type: 'prefill', L, ms: performance.now() - t0, scratchBytes: rt.estimatePrefillScratchBytes(L), pool: rt.poolStats() });
  }

  for (const ctx of [128, 1024, 4096, 7800]) {
    if (ctx >= rt.maxCtx) { row({ type: 'greedy-decode', ctx, skipped: 'ctx exceeds maxCtx' }); continue; }
    const r = await timeGreedy(rt, ref.ids, ctx, 32);
    row({ type: 'greedy-decode', ctx, selectedBatch: rt.MAXBATCH, tokens: r.tokens, seconds: r.seconds, tokPerSec: r.tokens / r.seconds, readbackBytesPerToken: 4 });
  }

  for (const k of [1, 2, 4, 8, 16, 32]) {
    if (k > rt.decodeBatchCapacity) { row({ type: 'batch-candidate', k, skipped: 'exceeds capacity' }); continue; }
  ...
  row({ type: 'sampling-topk', topK: 40, tokens: 8, seconds: sampleSeconds, tokPerSec: 8 / sampleSeconds, readbackBytesPerToken: 40 * 8 });
  ...
  if (adapterFiles) {
    ...
    row({ type: 'lora-greedy-decode', modules: Object.keys(lora.modules).length, tokens: r.tokens, seconds: r.seconds, tokPerSec: r.tokens / r.seconds });
...
window.addEventListener('DOMContentLoaded', () => window.run().catch(e => { row({ type: 'error', message: e.message, stack: (e.stack || '').slice(0, 500) }); console.log('VWG ERROR ' + e.message); }));
